...
public class CategoryDtoService(IMapper mapper, ICategoryRepository categoryRepository) : ICategoryDtoService
{
    ...
    public async Task DeleteAsync(int? id)
    {
        ...
        var getCategorytId = await _categoryRepository.GetByIdAsync(id);
        if (getCategorytId == null)
            throw new RequestException(new RequestError
            {
                Message = $"Category with id: {id} not found.",
                Severity = "Not found.",
        ...
        var removeCategory = await _categoryRepository.DeleteAsync(getCategorytId);
